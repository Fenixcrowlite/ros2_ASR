...
export function initBenchmarkPage(ctx) {
  ...
  function datasetProfileFromRegistryEntry(row) {
    const fromMetadata = String(row?.metadata?.dataset_profile || '').replace(/^datasets\//, '').trim();
    return fromMetadata || String(row?.dataset_id || '').trim();
  }

  function datasetSource(row) {
    return String(row?.metadata?.source || '').trim().toLowerCase();
  }

  function isUserDataset(row) {
    return ['manual', 'upload'].includes(datasetSource(row));
  }

  function isExternalCatalogDataset(row) {
    return datasetSource(row) === 'external_download';
  }

  function datasetProfileOptions(profileRows = [], datasetRows = []) {
    const registered = datasetRows || [];
    const userDatasets = registered
      .filter(isUserDataset)
      .map(datasetProfileFromRegistryEntry)
      .filter(Boolean)
      .reverse();
    const bundledOrLocalRegistered = registered
      .filter((row) => !isUserDataset(row) && !isExternalCatalogDataset(row))
      .map(datasetProfileFromRegistryEntry)
      .filter(Boolean);
    const catalogRegistered = registered
      .filter((row) => !isUserDataset(row) && isExternalCatalogDataset(row))
      .map(datasetProfileFromRegistryEntry)
      .filter(Boolean);
    const yamlProfiles = (profileRows || []).map((item) => String(item || '').trim()).filter(Boolean);
    return [...new Set([...userDatasets, ...bundledOrLocalRegistered, ...catalogRegistered, ...yamlProfiles])];
  }

  function preferredDatasetProfile(profileRows = [], datasetRows = []) {
    const current = datasetProfileSelect?.value || '';
    const options = datasetProfileOptions(profileRows, datasetRows);
    if (current && options.includes(current)) {
      return current;
    }
    const latestUserRegistered = [...(datasetRows || [])]
      .reverse()
      .filter((row) => row.valid !== false)
      .filter(isUserDataset)
      .map(datasetProfileFromRegistryEntry)
      .find(Boolean);
    if (latestUserRegistered && options.includes(latestUserRegistered)) {
      return latestUserRegistered;
    }
    const firstBundledOrLocal = (datasetRows || [])
      .filter((row) => row.valid !== false)
      .filter((row) => !isUserDataset(row) && !isExternalCatalogDataset(row))
      .map(datasetProfileFromRegistryEntry)
      .find(Boolean);
    if (firstBundledOrLocal && options.includes(firstBundledOrLocal)) {
      return firstBundledOrLocal;
    }
    return options[0] || '';
  }
  ...
  async function loadOptions() {
    const [benchmarkProfiles, datasetProfiles, datasetRegistry, metricsProfiles, providerProfilesResp, fetchedNoiseCatalog] = await Promise.all([
      api.profilesByType('benchmark'),
      api.profilesByType('datasets'),
      api.datasetsList().catch(() => ({ datasets: [] })),
    ...
    const datasetOptions = datasetProfileOptions(datasetProfiles.profiles || [], datasetRegistry.datasets || []);
    ui.updateSelectOptions(
      datasetProfileSelect,
      datasetOptions,
      preferredDatasetProfile(datasetProfiles.profiles || [], datasetRegistry.datasets || [])
    );
