...
function bestProvider(providerSummaries = []) {
  return [...providerSummaries]
    ...
    .sort((left, right) => {
      ...
      const leftLatency = metricValue(left, 'end_to_end_latency_ms') ?? Number.POSITIVE_INFINITY;
      const rightLatency = metricValue(right, 'end_to_end_latency_ms') ?? Number.POSITIVE_INFINITY;
...
export function initDashboardPage(ctx) {
    ...
    leaderboardRoot.innerHTML = ui.table(
      [
        ...
        { key: 'wer', label: 'WER', value: (row) => ui.escapeHtml(ui.formatNumber(metricValue(row, 'wer'))) },
        {
          ...
          label: 'Latency ms',
          value: (row) =>
            ui.escapeHtml(ui.formatNumber(metricValue(row, 'end_to_end_latency_ms'))),
        },
        {
          key: 'cost',
          label: 'Cost USD',
  ...
  function renderWhatChanged(payload) {
    ...
    const comparisons = [
      ...
      {
        label: 'Latency ms',
        latest: metricValue(latestBest || latest, 'end_to_end_latency_ms'),
        previous: metricValue(previousBest || previous, 'end_to_end_latency_ms'),
      },
      {
        label: 'Exact Match',
