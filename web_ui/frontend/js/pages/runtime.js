...
export function initRuntimePage(ctx) {
  ...
  function runtimeIsActive() {
    return ['starting', 'running', 'active', 'streaming'].includes(effectiveRuntimeState());
  }
  ...
  function setBusy(busy) {
    ...
    [stopButton, recognizeButton, sampleBrowseButton, generateNoiseButton].forEach((button) => {
      if (button) {
        button.disabled = busy;
      }
    });
    if (startButton) {
      startButton.disabled = busy || runtimeIsActive();
    }
    if (reconfigureButton) {
      const active = runtimeIsActive();
      reconfigureButton.disabled = busy || active;
      reconfigureButton.title = active
        ? 'Stop the current session before applying runtime configuration changes.'
        : 'Apply runtime configuration while the session is idle.';
    }
