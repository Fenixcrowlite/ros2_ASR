...
export function createApiClient() {
  return {
    ...
    datasetManualSample: (payload) =>
      request('/api/datasets/manual_sample', { method: 'POST', body: formBody(payload) }),
    datasetSampleUpdate: (id, sampleId, payload) =>
      request(`/api/datasets/${encodeURIComponent(id)}/samples/${encodeURIComponent(sampleId)}`, {
        method: 'PUT',
        body: jsonBody(payload),
      }),
    datasetDelete: (id, params = {}) =>
      request(buildPathWithQuery(`/api/datasets/${encodeURIComponent(id)}`, params), { method: 'DELETE' }),
