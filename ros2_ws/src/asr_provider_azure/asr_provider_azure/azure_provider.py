...
from asr_provider_azure.backend import AzureAsrBackend, AzureStreamingSession
...
def _endpoint_is_url(value: str) -> bool:
    text = str(value or "").strip().lower()
    return text.startswith(("https://", "http://", "wss://"))


def _backend_has_connection_target(backend: AzureAsrBackend) -> bool:
    method = getattr(backend, "has_connection_target", None)
    if callable(method):
        return bool(method())
    return bool(getattr(backend, "region", "") or _endpoint_is_url(getattr(backend, "endpoint", "")))
...
class AzureProvider(AsrProviderAdapter):
    ...
    def initialize(self, config: dict[str, Any], credentials_ref: dict[str, str]) -> None:
        merged = dict(config)
        if credentials_ref.get("AZURE_SPEECH_KEY"):
            merged["speech_key"] = credentials_ref["AZURE_SPEECH_KEY"]
        if credentials_ref.get("AZURE_SPEECH_REGION"):
            merged["region"] = credentials_ref["AZURE_SPEECH_REGION"]
        if credentials_ref.get("ASR_AZURE_ENDPOINT"):
    ...
    def validate_config(self) -> list[str]:
        ...
        if not _backend_has_connection_target(self._backend):
            errors.append(
                "Azure region is missing. Provide AZURE_SPEECH_REGION unless "
                "ASR_AZURE_ENDPOINT is a full endpoint URL."
            )
        sdk_loader = getattr(self._backend, "_load_sdk", None)
        if callable(sdk_loader) and not sdk_loader():
            errors.append(
                "Azure speech SDK is unavailable. Install `azure-cognitiveservices-speech`."
            )
        return errors
