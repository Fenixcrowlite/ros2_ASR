...
class AzureAsrBackend(AsrBackend):
    ...
    def has_credentials(self) -> bool:
        return bool(self.key and self.has_connection_target())

    def uses_endpoint_url(self) -> bool:
        return _is_endpoint_url(self.endpoint)

    def has_connection_target(self) -> bool:
        return bool(self.region or self.uses_endpoint_url())
    ...
    def create_stream_session(
        ...
        *,
        ...
        if not self.has_connection_target():
            raise RuntimeError(
                "Missing Azure region. Set AZURE_SPEECH_REGION or provide a full ASR_AZURE_ENDPOINT URL."
            )
    ...
    def recognize_once(self, request: AsrRequest) -> AsrResponse:
        ...
        if not self.has_connection_target():
            return AsrResponse(
                ...
                error_message=(
                    "Missing Azure region. Set AZURE_SPEECH_REGION or provide a full "
                    "ASR_AZURE_ENDPOINT URL."
                ),
