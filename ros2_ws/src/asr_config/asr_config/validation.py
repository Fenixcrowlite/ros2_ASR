...
def validate_metric_payload(payload: dict[str, Any]) -> list[str]:
    """Validate metric selection, threshold, and scoring payloads."""
    errors: list[str] = []
    ...
    if "scenarios" in payload:
        ...
        for scenario, thresholds in scenarios.items():
            ...
            if not scenario_id:
                errors.append("scenario ids must be non-empty strings")
            if not isinstance(thresholds, dict) or not thresholds:
                errors.append(f"scenarios.{scenario_id or '<empty>'} must be a non-empty object")
                continue
            for key, value in thresholds.items():
                threshold_name = str(key or "").strip()
                if not threshold_name:
                    errors.append(f"scenarios.{scenario_id}.threshold names must be non-empty")
                    continue
                if not threshold_name.endswith(("_min", "_max")):
                    errors.append(
                        f"scenarios.{scenario_id}.{threshold_name} must end with _min or _max"
                    )
                try:
                    numeric_value = float(value)
                except (TypeError, ValueError):
                    errors.append(f"scenarios.{scenario_id}.{threshold_name} must be numeric")
                    continue
                if not math.isfinite(numeric_value):
                    errors.append(f"scenarios.{scenario_id}.{threshold_name} must be finite")
        return errors

    if any(key in payload for key in ("scenario_weights", "component_weights", "normalization_bounds")):
        for key in ("scenario_weights", "component_weights", "normalization_bounds"):
            value = payload.get(key, {})
            if not isinstance(value, dict) or not value:
                errors.append(f"{key} must be a non-empty object")
        sensitivity_delta = payload.get("sensitivity_delta")
        if sensitivity_delta is not None:
            try:
                numeric_delta = float(sensitivity_delta)
            except (TypeError, ValueError):
                errors.append("sensitivity_delta must be numeric")
            else:
                if not math.isfinite(numeric_delta) or numeric_delta < 0:
                    errors.append("sensitivity_delta must be a finite value >= 0")
        return errors

    errors.append("metric profile must define metrics, scenarios, or score weighting sections")
    return errors
