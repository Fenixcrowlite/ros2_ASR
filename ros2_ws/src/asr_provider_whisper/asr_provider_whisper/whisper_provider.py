...
from asr_provider_base.models import ProviderAudio, ProviderMetadata, ProviderStatus
...
class WhisperProvider(AsrProviderAdapter):
    ...
    def initialize(self, config: dict[str, Any], credentials_ref: dict[str, str]) -> None:
        ...
        self._provider_metadata = ProviderMetadata(
            ...
            model_id=str(config.get("model_size", "") or ""),
            device=str(config.get("device", "") or ""),
            source="local",
            extra={"compute_type": str(config.get("compute_type", "") or "")},
        )
