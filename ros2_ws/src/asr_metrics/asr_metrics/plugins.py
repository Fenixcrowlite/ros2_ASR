...
class MetricContext:
    ...
    def __post_init__(self) -> None:
        """Backfill derived canonical latency and duration fields."""
...
class SerMetric(MetricPlugin):
    """Sentence error rate derived from exact-match sample accuracy."""
    ...
    def compute(self, context: MetricContext) -> float:
        ...
        return float(support.ser)


class MerMetric(MetricPlugin):
    """Match error rate from word-level alignment counts."""
    ...
    def compute(self, context: MetricContext) -> float:
        ...
        return float(support.mer)


class WilMetric(MetricPlugin):
    """Word information lost from word-level alignment counts."""
    ...
    def compute(self, context: MetricContext) -> float:
        ...
        return float(support.wil)


class WipMetric(MetricPlugin):
    """Word information preserved from word-level alignment counts."""
    ...
    def compute(self, context: MetricContext) -> float:
        ...
        return float(support.wip)


class HallucinationOnSilenceMetric(MetricPlugin):
    """Binary flag for non-empty hypothesis when normalized reference is empty."""

    name = "hallucination_on_silence"
    ...
    def compute(self, context: MetricContext) -> float:
        ...
        return float(support.hallucination_on_silence)


class MicroWerMetric(WerMetric):
    """Per-sample value used by summary aggregation to compute micro WER."""

    name = "micro_wer"


class MacroWerMetric(WerMetric):
    """Per-sample WER used by summary aggregation to compute macro WER."""

    name = "macro_wer"


class MicroCerMetric(CerMetric):
    """Per-sample value used by summary aggregation to compute micro CER."""

    name = "micro_cer"


class MacroCerMetric(CerMetric):
    """Per-sample CER used by summary aggregation to compute macro CER."""

    name = "macro_cer"


class TimeoutRateMetric(MetricPlugin):
    """Per-sample timeout flag; summaries may also derive it from error codes."""
...
DEFAULT_PLUGINS: dict[str, MetricPlugin] = {
    ...
    SerMetric.name: SerMetric(),
    MerMetric.name: MerMetric(),
    WilMetric.name: WilMetric(),
    WipMetric.name: WipMetric(),
    HallucinationOnSilenceMetric.name: HallucinationOnSilenceMetric(),
    MicroWerMetric.name: MicroWerMetric(),
    MacroWerMetric.name: MacroWerMetric(),
    MicroCerMetric.name: MicroCerMetric(),
    MacroCerMetric.name: MacroCerMetric(),
    TimeoutRateMetric.name: TimeoutRateMetric(),
}
