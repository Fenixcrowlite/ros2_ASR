...
from metrics.config import ObservabilityConfig
from metrics.models import PipelineTrace
...
class FileTraceExporter:
    ...
    def export_runtime_trace(self, trace: PipelineTrace) -> str:
        base_dir = Path(self.config.artifact_root) / "runtime_sessions" / (
            trace.session_id or "unknown_session"
        ) / "metrics"
        base_dir.mkdir(parents=True, exist_ok=True)
        json_path = base_dir / f"{trace.trace_id}.json"
    ...
    def export_benchmark_trace(self, *, run_dir: str, trace: PipelineTrace) -> str:
        base_dir = Path(run_dir) / "metrics" / "traces"
        base_dir.mkdir(parents=True, exist_ok=True)
        json_path = base_dir / f"{trace.trace_id}.json"
        trace.artifact_path = str(json_path)
        if self.config.export_json:
