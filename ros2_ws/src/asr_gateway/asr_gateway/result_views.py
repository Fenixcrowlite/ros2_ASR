...
It also contains the recovery logic used when a run has partial canonical
artifacts.
"""
...
def _provider_tradeoff_snapshot(
    ...
    for provider_summary in provider_summaries:
        ...
        _, latency_ms = _metric_value_for_candidates(
            provider_summary,
            ("end_to_end_latency_ms", "time_to_final_result_ms", "provider_compute_latency_ms"),
        )
        _, rtf = _metric_value_for_candidates(
            provider_summary,
            ("end_to_end_rtf", "provider_compute_rtf"),
        )
        snapshot.append(
            {
...
def _build_provider_rankings(summary: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    ranking_specs = [
        ...
        ("end_to_end_latency_ms", ("end_to_end_latency_ms", "time_to_final_result_ms", "provider_compute_latency_ms")),
        ("end_to_end_rtf", ("end_to_end_rtf", "provider_compute_rtf")),
        ...
        rows.append(
            {
                ...
                "metrics_semantics_version": summary.get("metrics_semantics_version", 2),
...
def run_detail(
    ...
    *,
    ...
    return {
        ...
        "summary": {
            **expanded_summary,
            "metrics_semantics_version": expanded_summary.get("metrics_semantics_version", 2),
...
def result_row(
    ...
    *,
    ...
    metrics_rows = read_json(run_dir_path / "metrics" / "results.json", [])
    if not isinstance(metrics_rows, list) or row_index < 0 or row_index >= len(metrics_rows):
        raise HTTPException(
            status_code=404,
            detail=f"Result row not found for run `{run_id}` at index `{row_index}`",
        )
    row = metrics_rows[row_index]
    if not isinstance(row, dict):
        raise HTTPException(
            status_code=404,
            detail=f"Result row not found for run `{run_id}` at index `{row_index}`",
    ...
    *,
    ...
    for detail in details:
        ...
        if isinstance(summary, Mapping):
            semantics_versions.add(int(summary.get("metrics_semantics_version", 2) or 2))
                ...
                subjects.append(
                    {
                        ...
                        "metrics_semantics_version": int(
                            provider_summary.get(
                                ...
                                summary.get("metrics_semantics_version", 2),
                            )
                            or 2
                        ),
                    }
        ...
        subjects.append(
            {
                ...
                "provider_id": "",
                "provider_preset": "",
                "metrics_semantics_version": int(summary.get("metrics_semantics_version", 2) or 2)
                if isinstance(summary, Mapping)
                else 2,
            }
        )
