...
from asr_gateway.result_views import (
    result_row as result_row_helper,
)
from asr_gateway.result_views import (
...
EDITABLE_DATASET_SOURCES = {"manual", "upload"}
...
if (PROJECT_ROOT / "web_ui" / "frontend").exists():
    ...
    app.mount(
        "/ui",
        NoCacheStaticFiles(directory=str(PROJECT_ROOT / "web_ui" / "frontend"), html=True),
        name="ui",
    )
...
def _ensure_default_huggingface_ref(ref_name: str) -> Path:
    """Create a missing default Hugging Face env ref without storing secret material."""
    ref_path = _secret_ref_path(ref_name)
    if ref_path.exists():
        return ref_path
    templates = {
        "huggingface_api_token": (
            "ref_id: secrets/huggingface_api_token\n"
        ...
        "huggingface_local_token": (
            "ref_id: secrets/huggingface_local_token\n"
    ...
    template = templates.get(ref_name)
    if template is None:
        return ref_path
    ref_path.parent.mkdir(parents=True, exist_ok=True)
    ref_path.write_text(template, encoding="utf-8")
    return ref_path
...
def _dataset_registry_entries() -> list[Any]:
    ...
    # ids. Bundled registries expose catalog/demo datasets to read-only UI flows.
...
def _primary_dataset_entries_by_id() -> dict[str, DatasetEntry]:
    if not DATASET_REGISTRY_PATH.exists():
        return {}
    registry = DatasetRegistry(str(DATASET_REGISTRY_PATH))
    return {entry.dataset_id: entry for entry in registry.list() if entry.dataset_id}


def _dataset_editability(entry: DatasetEntry) -> tuple[bool, str]:
    primary_entry = _primary_dataset_entries_by_id().get(entry.dataset_id)
    if primary_entry is None:
        return False, "Bundled dataset registry entries are read-only."
    source = str(primary_entry.metadata.get("source", "") or "").lower()
    if source not in EDITABLE_DATASET_SOURCES:
        return False, f"Dataset source `{source or 'unknown'}` is read-only."
    return True, ""
...
def _dataset_entries() -> list[dict[str, Any]]:
    ...
    for entry in _dataset_registry_entries():
        ...
        editable, read_only_reason = _dataset_editability(entry)
        detail = {
...
def _primary_dataset_entry(dataset_id: str) -> DatasetEntry:
    did = _clean_name(dataset_id, "dataset_id")
    registry = DatasetRegistry(str(DATASET_REGISTRY_PATH))
    for entry in registry.list():
        if entry.dataset_id == did:
            return entry
    raise HTTPException(
        status_code=404,
        detail=f"Dataset `{did}` is not in the editable project registry.",
    )


def _editable_primary_dataset_entry(dataset_id: str) -> DatasetEntry:
    entry = _primary_dataset_entry(dataset_id)
    source = str(entry.metadata.get("source", "") or "").lower()
    if source not in EDITABLE_DATASET_SOURCES:
        raise HTTPException(
            status_code=403,
            detail=(
                f"Dataset `{entry.dataset_id}` is read-only because source is "
                f"`{source or 'unknown'}`."
            ),
        )
    return entry
...
def _unregister_primary_dataset(dataset_id: str) -> None:
    did = _clean_name(dataset_id, "dataset_id")
    payload = json.loads(DATASET_REGISTRY_PATH.read_text(encoding="utf-8"))
    payload["datasets"] = [
        item
        for item in payload.get("datasets", [])
        if str(item.get("dataset_id", "") or "") != did
    ]
...
def _safe_project_child(path: Path, parent: Path) -> bool:
    try:
...
def _delete_dataset_artifacts(entry: DatasetEntry) -> list[str]:
    deleted: list[str] = []
    did = _clean_name(entry.dataset_id, "dataset_id")
    manifest_path = Path(_resolve_dataset_manifest_ref(entry.manifest_ref))
    profile_ref = str(entry.metadata.get("dataset_profile", "") or "").replace("datasets/", "", 1)

    if profile_ref:
        profile_path, _profile_id = _profile_path("datasets", profile_ref)
        if profile_path.exists():
            profile_payload = _read_yaml(profile_path)
            if str(profile_payload.get("dataset_id", "") or "") == did:
                profile_path.unlink()
                deleted.append(_project_relative(profile_path))

    if (
        _safe_project_child(manifest_path, PROJECT_ROOT / "datasets" / "manifests")
        and manifest_path.exists()
    ):
        manifest_path.unlink()
        deleted.append(_project_relative(manifest_path))

    imported_dir = PROJECT_ROOT / "datasets" / "imported" / did
    if imported_dir.exists() and _safe_project_child(imported_dir, PROJECT_ROOT / "datasets" / "imported"):
        shutil.rmtree(imported_dir)
        deleted.append(_project_relative(imported_dir))

    return deleted


def _ensure_dataset_profile(
    dataset_profile: str, manifest_path: str, default_language: str, dataset_id: str = ""
) -> tuple[str, str]:
    path, pid = _profile_path("datasets", dataset_profile)
    did = _clean_name(dataset_id or pid, "dataset_id")
    if path.exists():
        existing = _read_yaml(path)
        existing_dataset_id = str(existing.get("dataset_id", "") or "").strip()
        if existing_dataset_id and existing_dataset_id != did:
            raise HTTPException(
                status_code=409,
                detail=(
                    f"Dataset profile `{pid}` already points to dataset `{existing_dataset_id}`. "
                    f"Use profile `{did}` or choose a different profile name."
                ),
            )
    payload = {
...
def _wav_duration_sec(path: Path) -> float:
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Uploaded audio is not a valid WAV: {exc}") from exc
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_no, line in enumerate(handle, start=1):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Existing manifest has invalid JSON on line {line_no}: {exc}",
                ) from exc
...
def runtime_recognize_once(req: RecognizeRequest) -> dict[str, Any]:
    ...
    payload["asr_metrics"] = {
        "metrics_semantics_version": payload.get("metrics_semantics_version", 2),
...
def secret_huggingface_token_save(req: HuggingFaceTokenSaveRequest) -> dict[str, Any]:
    ref_name = _normalize_ref_name(req.ref_name)
    ref_path = _ensure_default_huggingface_ref(ref_name)
    if not ref_path.exists():
        raise HTTPException(status_code=404, detail=f"Secret ref not found: {ref_name}")
...
if MULTIPART_AVAILABLE:
    ...
    async def datasets_import_upload(
        ...
        try:
            ...
            resolved_profile_id, profile_path = _ensure_dataset_profile(
                profile_id, manifest_path, language or "en-US", did
            )
        except Exception as exc:
            raise HTTPException(status_code=400, detail=str(exc)) from exc
    ...
    @app.post("/api/datasets/manual_sample")
    async def datasets_manual_sample(
        dataset_id: str = Form(...),
        sample_id: str = Form(...),
        transcript: str = Form(...),
        dataset_profile: str = Form(""),
        ...
        did = _clean_name(dataset_id, "dataset_id")
        sid = _clean_name(sample_id, "sample_id")
        profile_id = _normalize_profile_id("datasets", dataset_profile or did)
        clean_transcript = str(transcript or "").strip()
        if not clean_transcript:
            raise HTTPException(status_code=400, detail="transcript is required")
        if len(clean_transcript) > 20000:
        ...
        dataset_root = PROJECT_ROOT / "datasets" / "imported" / did
        dataset_root.mkdir(parents=True, exist_ok=True)
        audio_path = dataset_root / f"{sid}.wav"
        ...
        manifest_path = PROJECT_ROOT / "datasets" / "manifests" / f"{did}.jsonl"
        manifest_path.parent.mkdir(parents=True, exist_ok=True)
        rows = _manifest_rows(manifest_path)
        existing_index = next(
            (index for index, row in enumerate(rows) if str(row.get("sample_id", "")) == sid),
            None,
        )
        if existing_index is not None and not replace_existing:
            ...
            raise HTTPException(
                status_code=409,
                detail=f"sample_id already exists in dataset `{did}`. Enable replace to overwrite it.",
            )
        ...
        row = {
            ...
            "split": str(split or "test").strip() or "test",
            "tags": ["manual", "recorded"],
            "metadata": {
                "source": "gui_manual_recording",
                "created_at": _now_iso(),
                "original_filename": str(audio_file.filename or "manual_sample.wav"),
            },
        }
        ...
        resolved_profile_id, profile_path = _ensure_dataset_profile(
            profile_id, str(manifest_path), row["language"], did
        )
        registry = DatasetRegistry(str(DATASET_REGISTRY_PATH))
        registry.register(
            DatasetEntry(
                ...
                metadata={"dataset_profile": f"datasets/{resolved_profile_id}", "source": "manual"},
            )
        )
        return {
            "success": True,
            "message": "Manual dataset sample saved",
            "dataset_id": did,
            "dataset_profile": f"datasets/{resolved_profile_id}",
            "sample_id": sid,
    ...
    @app.post("/api/datasets/manual_sample")
    async def datasets_manual_sample_unavailable() -> dict[str, Any]:
        raise HTTPException(
            status_code=503,
            detail='Manual dataset recording requires "python-multipart". Install dependencies from requirements.txt.',
        )
...
@app.put("/api/datasets/{dataset_id}/samples/{sample_id}")
def datasets_update_sample(
    dataset_id: str, sample_id: str, req: DatasetSampleUpdateRequest
) -> dict[str, Any]:
    entry = _editable_primary_dataset_entry(dataset_id)
    old_sample_id = _clean_name(sample_id, "sample_id")
    new_sample_id = _clean_name(req.sample_id or old_sample_id, "sample_id")

    manifest_path = Path(_resolve_dataset_manifest_ref(entry.manifest_ref)).resolve()
    if not _safe_project_child(manifest_path, PROJECT_ROOT / "datasets" / "manifests"):
        raise HTTPException(
            status_code=400,
            detail=f"Unsafe editable dataset manifest path: {manifest_path}",
        )
    if not manifest_path.exists():
        raise HTTPException(status_code=404, detail=f"Dataset manifest not found: {manifest_path}")
    ...
    target_index = next(
        (
            ...
            if str(row.get("sample_id", "") or "") == old_sample_id
        ),
        None,
    )
    if target_index is None:
        raise HTTPException(
            status_code=404,
            detail=f"Sample `{old_sample_id}` not found in dataset `{entry.dataset_id}`.",
        )
    if new_sample_id != old_sample_id and any(
        str(row.get("sample_id", "") or "") == new_sample_id for row in rows
    ):
        raise HTTPException(
            status_code=409,
            detail=f"sample_id already exists in dataset `{entry.dataset_id}`: {new_sample_id}",
        )

    row = dict(rows[target_index])
    clean_transcript = str(req.transcript or row.get("transcript", "") or "").strip()
    if not clean_transcript:
        raise HTTPException(status_code=400, detail="transcript is required")
    if len(clean_transcript) > 20000:
        raise HTTPException(status_code=400, detail="transcript is too long")
    ...
    row["transcript"] = clean_transcript
    metadata = row.get("metadata", {})
    if not isinstance(metadata, dict):
        metadata = {}
    metadata["updated_at"] = _now_iso()
    metadata["updated_by"] = "gui_dataset_editor"
    row["metadata"] = metadata
    rows[target_index] = row
    _write_manifest_rows(manifest_path, rows)

    # Re-load through the canonical parser so the response reflects the same
    # validation and path resolution used by benchmark execution.
    samples = load_manifest(str(manifest_path))
    registry = DatasetRegistry(str(DATASET_REGISTRY_PATH))
    registry.register(
        DatasetEntry(
    ...
    updated = next(sample for sample in samples if sample.sample_id == new_sample_id)
    return {
        "success": True,
        "message": "Dataset sample updated",
        "dataset_id": entry.dataset_id,
        "sample_id": updated.sample_id,
        "previous_sample_id": old_sample_id,
        "renamed": new_sample_id != old_sample_id,
        "manifest_path": str(manifest_path),
        "sample_count": len(samples),
        "sample": {
            "sample_id": updated.sample_id,
...
def datasets_delete(dataset_id: str, delete_artifacts: bool = Query(True)) -> dict[str, Any]:
    entry = _editable_primary_dataset_entry(dataset_id)

    deleted: list[str] = []
    if delete_artifacts:
        deleted = _delete_dataset_artifacts(entry)
    _unregister_primary_dataset(entry.dataset_id)
    return {
        "success": True,
        "message": f"Dataset `{entry.dataset_id}` deleted",
        "dataset_id": entry.dataset_id,
        "deleted_artifacts": deleted,
        "registry_path": str(DATASET_REGISTRY_PATH),
    }
