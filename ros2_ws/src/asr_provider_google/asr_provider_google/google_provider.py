"""Google cloud provider adapter with native streaming sessions."""

from __future__ import annotations

from typing import Any

from asr_provider_google.backend import GoogleAsrBackend, GoogleStreamingSession
...
def _normalize(
    ...
    if not is_partial and not degraded and not text:
        ...
        error_message = error_message or "Google returned an empty transcript."
...
class GoogleProvider(AsrProviderAdapter):
    ...
    def __init__(self) -> None:
        ...
        self._stream_session: GoogleStreamingSession | None = None
    ...
    def push_audio(self, chunk: bytes) -> NormalizedAsrResult | None:
        if self._stream_session is None:
            raise RuntimeError("Google stream is not active")
