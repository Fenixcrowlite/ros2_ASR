...
ARCHVIZ := ./archviz
ARCHVIZ_OUT := artifacts/archviz
# Keep CMake on ROS Jazzy's system Python. Forcing Python3_EXECUTABLE to the
# project venv breaks rosidl_generator_py and can produce empty message modules.
...
.PHONY: help setup setup-vosk build docs-check test test-unit test-ros test-colcon test-gateway-smoke run bench bench-suite collect-metrics validate-datasets report web-gui web-gui-lan web-gui-stop up up-runtime up-lan down hf-smoke-local hf-smoke-api bench-hf rqt arch-static arch-runtime arch arch-diff lint lint-ruff lint-mypy security-scan format clean dist
...
bench-hf:
	bash -lc 'source $(VENV)/bin/activate && python3 scripts/run_benchmark_core.py --benchmark-profile huggingface_provider_matrix --configs-root "$(CURDIR)/configs" --artifact-root "$(CURDIR)/artifacts" --registry-path "$(CURDIR)/datasets/registry/datasets.json"'
...
arch-static:
	bash -lc "source $(VENV)/bin/activate && $(ARCHVIZ) static --ws ros2_ws --out $(ARCHVIZ_OUT)"

arch-runtime:
	bash -lc "source $(VENV)/bin/activate && if [ -f $(ROS_SETUP) ]; then source $(ROS_SETUP); [ -f ros2_ws/install/setup.bash ] && source ros2_ws/install/setup.bash || true; else echo 'ROS2 Jazzy setup not found at $(ROS_SETUP); runtime extractor will record it as runtime error.'; fi; $(ARCHVIZ) runtime --ws ros2_ws --out $(ARCHVIZ_OUT) --profile full --timeout-sec 20"

arch:
	bash -lc "source $(VENV)/bin/activate && if [ -f $(ROS_SETUP) ]; then source $(ROS_SETUP); [ -f ros2_ws/install/setup.bash ] && source ros2_ws/install/setup.bash || true; else echo 'ROS2 Jazzy setup not found at $(ROS_SETUP); runtime extractor will record it as runtime error.'; fi; $(ARCHVIZ) all --ws ros2_ws --out $(ARCHVIZ_OUT) --profile full --timeout-sec 20"

arch-diff:
	bash -lc "source $(VENV)/bin/activate && $(ARCHVIZ) diff --a $(ARCHVIZ_OUT)/merged_graph_prev.json --b $(ARCHVIZ_OUT)/merged_graph.json --out $(ARCHVIZ_OUT)/CHANGELOG_ARCH.md"
...
clean:
	...
	rm -rf results reports/benchmarks artifacts/benchmark_runs/* artifacts/runtime_sessions/* artifacts/exports/* artifacts/temp/*
