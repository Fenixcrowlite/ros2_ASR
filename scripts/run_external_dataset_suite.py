...
def _write_outputs(payload: dict[str, Any], suite_id: str) -> None:
        ...
        lines.append(
            ...
            + " | ".join(
                [
                    ...
                    f"{float(mean_metrics.get('end_to_end_latency_ms', 0.0) or 0.0):.2f}",
                    f"{float(mean_metrics.get('end_to_end_rtf', 0.0) or 0.0):.4f}",
                ]
            )
            + " |"
        )
    ...
    for acoustic_profile, items in sorted(by_acoustic.items()):
        lines.append(
            ...
            f"latency `{_mean_metric(items, 'end_to_end_latency_ms'):.2f}` ms, "
            f"RTF `{_mean_metric(items, 'end_to_end_rtf'):.4f}`"
        )
