#!/usr/bin/env python3
"""Sanitized credential discovery for cloud provider checks."""
...
def _discover_aws_bucket(values: dict[str, str]) -> tuple[str, str, str]:
    ...
    try:
        ...
        preferred_tokens = ("asr", "transcribe", "speech", "ros", "benchmark")
        buckets = sorted(
            buckets,
            key=lambda name: (
...
def write_credential_reports(
    *,
    output_json: Path = PROJECT_ROOT / "reports" / "credential_discovery" / "credential_availability.json",
    output_md: Path = PROJECT_ROOT / "reports" / "credential_discovery" / "credential_availability.md",
...
if __name__ == "__main__":
    ...
    print(json.dumps({"providers": len(result["providers"]), "output": "reports/credential_discovery"}, indent=2))
