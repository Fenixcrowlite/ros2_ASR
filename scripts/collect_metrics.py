#!/usr/bin/env python3
# ruff: noqa: E402, I001
"""Build schema-first benchmark artifacts from ASR run outputs."""
...
def _default_input(results_dir: Path) -> Path:
    raise SystemExit(
        "No input was provided. Pass --input with a canonical artifacts/benchmark_runs/<run_id> "
        "directory, canonical reports/summary.json, or schema-first summary directory."
    )
...
def _load_payload(
    *,
    ...
    if isinstance(payload, list):
        raise SystemExit(
            "Flat benchmark result lists are no longer supported. Regenerate the run with "
            "scripts/run_benchmark_core.py or pass a canonical artifacts/benchmark_runs/<run_id> directory."
        )
    ...
    raise SystemExit(
        "Unsupported input schema. Expected canonical summary JSON, canonical run directory, "
        "or schema-first run directory."
    )
...
def _normalize_rows(
    *,
    ...
    for index, row in enumerate(loaded.rows, start=1):
            ...
            output_rows.append(
                {
                    **{field: row.get(field, "") for field in UTTERANCE_FIELDS},
                    ...
                    "mer": _as_float(row.get("mer")) if _as_float(row.get("mer")) is not None else support["mer"],
                    "wil": _as_float(row.get("wil")) if _as_float(row.get("wil")) is not None else support["wil"],
                    "wip": _as_float(row.get("wip")) if _as_float(row.get("wip")) is not None else support["wip"],
                    "word_edits": int(support["word_edits"]),
                    "ref_words": int(support["reference_word_count"]),
                    "word_hits": int(support["word_hits"]),
                    "word_substitutions": int(support["word_substitutions"]),
                    "word_deletions": int(support["word_deletions"]),
                    "word_insertions": int(support["word_insertions"]),
                    "char_edits": int(support["char_edits"]),
                    "ref_chars": int(support["reference_char_count"]),
                    "char_hits": int(support["char_hits"]),
                    "char_substitutions": int(support["char_substitutions"]),
                    "char_deletions": int(support["char_deletions"]),
                    "char_insertions": int(support["char_insertions"]),
                    "hallucination_on_silence": support["hallucination_on_silence"],
                    "ftl_ms": ftl_ms,
        ...
        final_latency_ms = _metric(row, "time_to_final_result_ms", "end_to_end_latency_ms", "latency_ms")
        ...
        output_rows.append(
            {
                ...
                "word_hits": int(support.get("word_hits", support.get("hits", 0)) or 0),
                "word_substitutions": int(
                    support.get("word_substitutions", support.get("substitutions", 0)) or 0
                ),
                "word_deletions": int(
                    support.get("word_deletions", support.get("deletions", 0)) or 0
                ),
                "word_insertions": int(
                    support.get("word_insertions", support.get("insertions", 0)) or 0
                ),
                "char_edits": int(support.get("char_edits", 0) or 0),
                "ref_chars": int(support.get("reference_char_count", 0) or 0),
                "char_hits": int(support.get("char_hits", 0) or 0),
                "char_substitutions": int(support.get("char_substitutions", 0) or 0),
                "char_deletions": int(support.get("char_deletions", 0) or 0),
                "char_insertions": int(support.get("char_insertions", 0) or 0),
                "hallucination_on_silence": support["hallucination_on_silence"],
                "ftl_ms": ftl_ms,
                "final_latency_ms": final_latency_ms,
                "provider_compute_rtf": provider_compute_rtf,
    ...
    *,
    ...
    for (backend, model, execution_mode), group_rows in sorted(grouped.items()):
        ...
        summary = {
            ...
            "mer": _mean_optional([_as_float(row.get("mer")) for row in group_rows]),
            "wil": _mean_optional([_as_float(row.get("wil")) for row in group_rows]),
            "wip": _mean_optional([_as_float(row.get("wip")) for row in group_rows]),
            "micro_wer": _sum_rate(group_rows, "word_edits", "ref_words", "wer"),
            "macro_wer": _mean_optional(
                [
                    _as_float(row.get("wer"))
                    for row in group_rows
                    if int(row.get("ref_words") or 0) > 0
                ]
            ),
            "micro_cer": _sum_rate(group_rows, "char_edits", "ref_chars", "cer"),
            "macro_cer": _mean_optional(
                [
                    _as_float(row.get("cer"))
                    for row in group_rows
                    if int(row.get("ref_chars") or 0) > 0
                ]
            ),
            "hallucination_on_silence": _mean_optional(
                [
                    _as_float(row.get("hallucination_on_silence"))
                    for row in group_rows
                    if int(row.get("ref_words") or 0) == 0
                ]
            ),
...
def _build_subset_rows(rows: list[dict[str, Any]], run_id: str) -> list[dict[str, Any]]:
    group_fields = ["dataset", "language", "noise_profile", "speaker_id"]
    ...
    for key, group_rows in sorted(grouped.items()):
        dataset, language, noise_profile, speaker_id = key
        ser = _mean_optional([_as_float(row.get("ser")) for row in group_rows])
        subset_rows.append(
            {
                "run_id": run_id,
                ...
                "cer": _sum_rate(group_rows, "char_edits", "ref_chars", "cer"),
                "ser": ser,
                "sample_accuracy": 1.0 - ser if ser is not None else None,
                "end_to_end_rtf": _mean_optional(
                    [_as_float(row.get("end_to_end_rtf")) for row in group_rows]
                ),
                "provider_compute_rtf": _mean_optional(
                    [_as_float(row.get("provider_compute_rtf")) for row in group_rows]
                ),
                "hallucination_on_silence": _mean_optional(
                    [
                        _as_float(row.get("hallucination_on_silence"))
    ...
    *,
    ...
    return {
        ...
        "artifacts": {
            ...
            "per_sample_csv": _repo_relative_path(output_dir / "per_sample_metrics.csv"),
            "per_sample_parquet": _repo_relative_path(output_dir / "per_sample_metrics.parquet"),
            "subset_csv": _repo_relative_path(output_dir / "subset_metrics.csv"),
...
def _write_markdown_report(path: Path, manifest: dict[str, Any], summary_rows: list[dict[str, Any]]) -> None:
    ...
    lines.extend(
        [
            ...
            f"- per-sample metrics: `{manifest['artifacts']['per_sample_csv']}`",
            f"- subset metrics: `{manifest['artifacts']['subset_csv']}`",
...
def main() -> None:
    ...
    parser.add_argument("--input", default="", help="Canonical summary JSON, schema-first summary JSON, or run directory")
    parser.add_argument("--results-json", default="", help="Explicit canonical results JSON path")
    ...
    subset_rows = _build_subset_rows(utterance_rows, run_id)
    ...
    _write_csv(output_dir / "per_sample_metrics.csv", UTTERANCE_FIELDS, utterance_rows)
    _write_optional_parquet(output_dir / "per_sample_metrics.parquet", utterance_rows)
    _write_csv(
        output_dir / "subset_metrics.csv",
        [
            "run_id",
            "dataset",
            "language",
