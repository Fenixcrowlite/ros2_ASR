...
Runs the existing benchmark core, then builds schema-first benchmark artifacts in
...
Options:
  ...
  --registry-path PATH
  --results-dir PATH
  --input PATH              Existing canonical or schema-first input for --skip-benchmark
...
if [[ "$SKIP_BENCHMARK" -eq 0 ]]; then
  ...
  BENCH_OUTPUT="$("${BENCH_ARGS[@]}")"
  printf '%s\n' "$BENCH_OUTPUT"
  INPUT_PATH="$(python3 -c 'import json,sys; print(json.load(sys.stdin)["run_dir"])' <<< "$BENCH_OUTPUT")"
else
  if [[ -z "$INPUT_PATH" ]]; then
    INPUT_PATH="$(
      find "$ARTIFACT_ROOT/benchmark_runs" -mindepth 1 -maxdepth 1 -type d -printf '%T@ %p\n' \
        | sort -nr \
        | awk 'NR==1{print $2}'
    )"
    if [[ -z "$INPUT_PATH" ]]; then
      echo "No canonical benchmark run found under $ARTIFACT_ROOT/benchmark_runs." >&2
      exit 1
    fi
  fi
fi
