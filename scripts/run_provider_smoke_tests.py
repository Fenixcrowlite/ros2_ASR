#!/usr/bin/env python3
"""Run provider availability and one-utterance smoke checks."""
...
def main() -> None:
    ...
    parser.add_argument("--output", default="results/provider_smoke_tests.csv")
    parser.add_argument(
        "--dataset-manifest",
        default="datasets/manifests/sample_dataset.jsonl",
        help=(
            "JSONL manifest used for provider recognition checks. The default "
            "uses the bundled sample WAV; pass an imported dataset manifest for "
            "larger provider probes."
        ),
    )
