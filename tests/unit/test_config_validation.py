...
def test_validate_metric_payload_accepts_threshold_profiles() -> None:
    errors = validate_metric_payload(
        {
            "scenarios": {
                "embedded": {
                    "end_to_end_rtf_mean_max": 0.8,
...
def test_validate_metric_payload_accepts_score_weight_profiles() -> None:
    errors = validate_metric_payload(
        {
            "scenario_weights": {"embedded": {"quality": 1.0}},
            "component_weights": {"quality": {"wer": 1.0}},
            "normalization_bounds": {"wer": {"direction": "lower", "target": 0.05, "worst": 0.6}},
...
def test_validate_metric_payload_rejects_unknown_metric_profile_shape() -> None:
    errors = validate_metric_payload({"profile_id": "metrics/broken"})

    assert "metric profile must define metrics, scenarios, or score weighting sections" in errors
