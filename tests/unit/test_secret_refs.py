...
def test_missing_project_relative_file_secret_ref_keeps_project_path(tmp_path: Path) -> None:
    project_root = tmp_path / "project"
    refs_dir = project_root / "secrets" / "refs"
    refs_dir.mkdir(parents=True)
    ref_path = refs_dir / "google.yaml"
    ...
    assert str(project_root / "secrets" / "google" / "service-account.json") in str(
        exc_info.value
    )
    assert "secrets/refs/secrets" not in str(exc_info.value)
