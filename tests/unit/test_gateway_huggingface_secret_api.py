...
def test_gateway_huggingface_token_save_creates_missing_default_ref(
    repo_root: Path,
    tmp_path: Path,
    monkeypatch,
) -> None:
    project_root = clone_project_layout(repo_root, tmp_path / "project")
    missing_ref = project_root / "secrets" / "refs" / "huggingface_api_token.yaml"
    missing_ref.unlink()
    ...
    assert saved["saved"] is True
    assert missing_ref.exists()
    assert "provider: huggingface_api" in missing_ref.read_text(encoding="utf-8")
