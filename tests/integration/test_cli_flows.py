...
def test_collect_metrics_cli_reads_canonical_artifacts(repo_root: Path, tmp_path: Path) -> None:
    ...
    assert "# ASR Benchmark Report" in report_md.read_text(encoding="utf-8")


def test_collect_metrics_cli_rejects_flat_result_lists(repo_root: Path, tmp_path: Path) -> None:
    flat_json = tmp_path / "benchmark_results.json"
    ...
    assert result.returncode != 0
    assert "Flat benchmark result lists are no longer supported" in result.stderr


def test_generate_report_cli_uses_canonical_summary_and_input_plot_directory(
        ...
        json.dumps(
            {
                ...
                "provider_summaries": [
                    {
                        "provider_key": "providers/whisper",
                        "provider_profile": "providers/whisper",
                        "provider_id": "whisper",
                        "provider_preset": "",
                        "quality_metrics": {
                            "wer": 0.25,
    ...
    assert "| providers/whisper | 0.250 | 0.250 | 0.500 | 20.0 | 1.000 | 1.000 | 0.3000 |" in report_text
...
def test_run_benchmark_core_cli_exports_canonical_artifacts(
    ...
    assert payload["metrics_semantics_version"] == 2
    assert payload["canonical_summary_json"].endswith("reports/summary.json")
    assert payload["canonical_results_json"].endswith("metrics/results.json")
