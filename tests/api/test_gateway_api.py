...
def test_benchmark_preview_audio_supports_clean_and_noise_variants(
    ...
    optional_fleurs_audio = (
        project_root
        / "datasets"
        / "imported"
        / "fleurs_en_us_test_subset"
        / "fleurs_en_us_12741024238657315067.wav"
    )
    if optional_fleurs_audio.exists():
        assert fleurs.status_code == 200
        assert fleurs.headers["content-type"].startswith("audio/wav")
    else:
        assert fleurs.status_code == 404
        assert "Dataset audio not found" in fleurs.json()["detail"]
...
def test_benchmark_history_status_compare_and_export(
    ...
    assert "end_to_end_rtf" in live_row["latency_metrics"]
...
def test_results_row_audio_replay_supports_clean_and_noise_preview(
    ...
    seed_rows = json.loads(results_path.read_text(encoding="utf-8"))
    expanded_rows = [
        {**seed_rows[0], "sample_id": f"sample_vosk_numbers_{index}"}
        for index in range(105)
    ]
    results_path.write_text(json.dumps(expanded_rows, ensure_ascii=True), encoding="utf-8")

    replay_beyond_results_head = client.get(
        "/api/results/runs/bench_seed_a/rows/104/audio",
        params={"source": "clean", "start_sec": 0.0, "duration_sec": 0.2},
    )
    assert replay_beyond_results_head.status_code == 200
    assert replay_beyond_results_head.headers["content-type"].startswith("audio/wav")
    assert noisy.content != clean.content
    ...
    class _FakeAwsBackend:
        def auth_status(self):
            return {
                ...
                "message": "IAM Identity Center sign-in is required.",
                "sso_session_expires_at": "",
                "sso_session_valid": False,
                "role_credentials_expires_at": "",
                "role_credentials_valid": False,
                "login_command": "aws sso login --profile ros2ws",
                "sso_start_url": "https://d-c36767bf7f.awsapps.com/start",
            }
...
def test_dataset_sample_update_edits_manifest_row(
    repo_root: Path, tmp_path: Path, monkeypatch
) -> None:
    _gateway_api, _fake_ros, client, project_root = _make_client(repo_root, tmp_path, monkeypatch)
    manifest_path = project_root / "datasets" / "manifests" / "editable_demo.jsonl"
    manifest_path.parent.mkdir(parents=True, exist_ok=True)
    manifest_rows = [
        {
            "sample_id": "sample_old",
            "audio_path": "../../data/sample/vosk_test.wav",
            "transcript": "old reference",
            "language": "en-US",
            "duration_sec": 1.0,
            "split": "test",
            "tags": ["manual"],
    ...
    registry["datasets"].append(
        {
            "dataset_id": "editable_demo",
            "manifest_ref": str(manifest_path),
            "sample_count": 2,
            "metadata": {"dataset_profile": "datasets/editable_demo", "source": "manual"},
        }
    )
    registry_path.write_text(json.dumps(registry), encoding="utf-8")

    response = client.put(
        "/api/datasets/editable_demo/samples/sample_old",
        json={"sample_id": "sample_new", "transcript": "new reference text"},
    )

    assert response.status_code == 200
    payload = response.json()
    assert payload["renamed"] is True
    assert payload["sample"]["sample_id"] == "sample_new"
    assert payload["sample"]["transcript"] == "new reference text"
    rows = [json.loads(line) for line in manifest_path.read_text(encoding="utf-8").splitlines()]
    assert rows[0]["sample_id"] == "sample_new"
    assert rows[0]["transcript"] == "new reference text"
    assert rows[0]["metadata"]["updated_by"] == "gui_dataset_editor"

    duplicate = client.put(
        "/api/datasets/editable_demo/samples/sample_new",
        json={"sample_id": "sample_other", "transcript": "still valid"},
    )
    assert duplicate.status_code == 409
    assert "sample_id already exists" in duplicate.json()["detail"]
